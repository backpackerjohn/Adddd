...
- R4 (12:50 UTC): The monthly spend limit is flapping — it killed 3/9 agents in the teardown run and 10/11 in the specs run, then a probe agent succeeded minutes later. Strategy adopted: commit completed artifacts immediately after every workflow, probe with a minimal haiku agent, then resume the same run ID (cached agents replay free; only failed agents re-run). No work is lost to the flapping; it only costs wall-clock.
